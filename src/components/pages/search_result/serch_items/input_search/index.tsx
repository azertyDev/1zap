...
export const InputSearch: FC<{
    valResert: (
        nextState?: Partial<FormikState<{ searchVal: string }>> | undefined
    ) => void;
    values: FieldInputProps<any>;
    fun: (e?: FormEvent<HTMLFormElement> | undefined) => void;
}> = ({ values, valResert, fun }): JSX.Element => {
    const { t } = useTranslation();

    const resertForm = useCallback(() => {
        return () => valResert();
    }, []);

    return (
        <form className={s.input_wr} onSubmit={fun}>
            <input
                {...values}
                type={'text'}
            ...
            <div className={s.close_res_input} onClick={resertForm()}>
                <Image
                    src={'/assets/icons/cleaninput.svg'}
                    alt={'icon'}
                    width={18}
                    height={18}
                />
